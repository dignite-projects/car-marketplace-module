...
export class CarMarketplaceComponent {
  ...
  requestImage(input, formData) {
    return new Promise((resolve, reject) => {
      const req = new HttpRequest('POST', `${environment.apis.FileExplorer.url}/api/file-explorer/files?containerName=${input.containerName}&entityId=${input.entityId}`, formData, {
      });
      this.http
        .request(req)
